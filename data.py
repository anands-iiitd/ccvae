...
def image_from_npy(data_path, image_name, transforms):
    ...
    image_path = os.path.join(data_path + "_images", image_name + '.npy')
...
class PairedDataset(Dataset):
    ...
    def __init__(self, data_path, transforms):
        csv_path = os.path.join(data_path + "_image_paths.csv")
        ...
        for i in image_names.size():
            self.images[i] = image_from_npy(data_path=data_path,
                                            image_name=image_names[i],
                                            transforms=transforms)
            self.label_index_map[self.labels[i]].append(i)
...
class PairedDataloader:
    ...
    def __init__(self, data_path, transforms, batch_size, device):
