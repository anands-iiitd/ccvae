...
class Encoder(nn.Module):
    def __init__(self, style_dim, class_dim):
        ...
        # Style embeddings
        self.style_mu = nn.Linear(in_features=256, out_features=style_dim,
                                  bias=True)
        self.style_logvar = nn.Linear(in_features=256, out_features=style_dim,
                                      bias=True)

        # Class embeddings
        self.class_output = nn.Linear(in_features=256, out_features=class_dim,
                                      bias=True)
...
class Decoder(nn.Module):
    def __init__(self, style_dim, class_dim):
        ...
        self.style_input = nn.Linear(in_features=style_dim, out_features=256,
                                     bias=True)

        # Class embeddings input
        self.class_input = nn.Linear(in_features=class_dim, out_features=256,
                                     bias=True)

        self.deconv_model = nn.Sequential(OrderedDict([
            ('deconvolution_1',
             nn.ConvTranspose2d(in_channels=128, out_channels=32, kernel_size=4,
            ...
            ('deconvolution_3',
             nn.ConvTranspose2d(in_channels=16, out_channels=1, kernel_size=4,
                                stride=2, padding=1, bias=True)),
            ('sigmoid_final', nn.Sigmoid())
        ]))

    def forward(self, style_embeddings, class_embeddings):
        style_embeddings = F.leaky_relu_(self.style_input(style_embeddings),
                                         negative_slope=0.2)
        class_embeddings = F.leaky_relu_(self.class_input(class_embeddings),
                                         negative_slope=0.2)
...
if __name__ == '__main__':
    ...
    loader = cycle(DataLoader(mnist, batch_size=64, shuffle=True, num_workers=0,
                              drop_last=True))
